...
# Part One
horizontal_vertical = list(
    filter(
        lambda line: line[0][0] == line[1][0] or line[0][1] == line[1][1], lines.copy()
    )
)
...
def fill_column(start, end) -> None:
    for i in range(start[1], end[1] + 1):
        cartesian[i][start[0]] += 1


def fill_row(start, end) -> None:
    for i in range(start[0], end[0] + 1):
        cartesian[start[1]][i] += 1
...
def fill_top_right(start, delta_x) -> None:
    for i in range(0, delta_x + 1):
        cartesian[start[1] - i][start[0] + i] += 1
